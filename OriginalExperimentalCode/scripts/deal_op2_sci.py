...
def read_results(dataset, metric_name, op_args):
    ...
    for obj_arg in ['hidden_num', 'lr', 'drop_rate']:
        ...
        if dataset == 'lung' and obj_arg == 'drop_rate':
            ...
            y = y[:-1]
            plt.figure(figsize=(10, 6))  # 设置图形大小
            plt.plot(x, y, marker='o', linestyle='-', color='b', linewidth=6, markersize=18)  # 折线图，带圆圈标记
            ...
            # 显示图形
            plt.savefig(save_dir / f'{dataset}_op_{obj_arg}_{metric_name}.png')
            plt.show()
            ...
            x = [1,2,3,4,5]
            plt.xticks(x, [0.0002,0.002, 0.01, 0.02, 0.03],fontsize=36)
            plt.yticks(fontsize=36)
...
if __name__ == '__main__':
    read_results(dataset='lung', metric_name='recall', op_args=[2200, 0.01, 'False', 0.6])
    read_results(dataset='lung', metric_name='precision', op_args=[2200, 0.01, 'False', 0.6])
