...
class Comparator(abc.ABC):
    ...
    #     outcome = {}
    #
    #     for model_name, (model_class, param_grid) in tqdm.tqdm( self.model_param_grid.items()):
    #
    #         if self.only_model:
    ...
    def run2(self, data_tuple):
        """

        :param data_tuple:
        :return:
        """
        model_name, (model_class, param_grid) = data_tuple
        ...
        if os.path.exists(save_path):
            ...
            print(f'start:{param_grid}')
            out = self.opt_model(model_class, param_grid, model_name=model_name)
            out.to_excel(save_path)
            print(f'end:{param_grid}')
            return None
    ...
    def run(self):
        ...
        with Pool(processes=self.process_num) as pool:
            results = list(tqdm.tqdm(pool.imap(self.run2, self.model_param_grid.items()), total=1530))
        # for model_name, (model_class, param_grid) in tqdm.tqdm(self.model_param_grid.items()):
        #
        #     run2((model_name, (model_class, param_grid)))
